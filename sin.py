...
with torch.no_grad():
    y_pred = model(x_pred)
    s = [10 for _ in range(len(x_pred))]

plt.scatter(x_train.numpy(), y_train.numpy(), label="True values", color="blue")
plt.plot(x_pred.numpy(), y_pred.numpy(), label="Predicted values", color="red")
plt.scatter(x_pred.numpy(), y_pred.numpy(), label="Predicted values", color="red", s=s)
plt.title("True vs Predicted (y = sin)")
plt.xlabel("x")
plt.ylabel("y")
plt.legend()
plt.show()
